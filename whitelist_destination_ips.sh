...
# whitelist_destination_ips.sh: whitelists a line delineated list of IPv4 addresses as destinations
...
whitelist_ip_as_destination() {
    echo curl -XPOST "http://${HUNTER_INTERNAL_IP}:8080/api/v0/dnscat2-ja3/whitelist/add" --data-binary @- >&2
    echo { "name": "$1", "type": "ip", "comment": "$wl_comment", "ip": { "ip": "$1", "src": false, "dst": true } } EOF >&2
...
for ip in `cat $1`; do
    if [ -n "$ip" ]; then
        ...
        whitelist_ip_as_destination "$ip"
        printf "\n"
    fi
done
